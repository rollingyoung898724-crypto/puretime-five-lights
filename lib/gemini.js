...
function buildPrompt({ prayerName, selectedState }, { isRetry = false } = {}) {
...
Write the body entirely as the user's own private reflection. Its first word must be "I" or "My", and it must naturally use first-person words such as I, my, or me.
Do not address the reader or generalize about others. Do not use "you", "your", "people", "Muslims", or similar generalized expressions in the body.
Do not give advice, instructions, lessons, encouragement directed at someone else, or religious teaching. Describe only what I notice, feel, remember, or intend for myself.
${isRetry ? 'The previous response failed the required format or first-person safety check. Rewrite it from scratch and follow every constraint exactly.' : ''}`;
}
    ...
    try {
      const interaction = await client.interactions.create({
        ...
        input: [
          ...
          { type: 'text', text: buildPrompt(input, { isRetry: attempt > 0 }) }
        ],
        response_format: {
          type: 'text',
